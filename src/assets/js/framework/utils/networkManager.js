...
export class NetworkManager {
	...
	async doFileRequest(route, method, formData) {
		...
		if (!formData instanceof FormData) {
			console.log(
				"Warning: you are doing a file request but data passed is not of type FormData"
			);
		}
	...
	#onFail(errorCode, error) {
		if (errorCode === 1000 && error === null) {
			...
			error =
				"Request did not arrive at server or no valid json response received from server. \n " +
				"Check Terminal in IntelliJ for server errors as well";
		}

		console.error(
			`Error with response code: ${errorCode} and message: ${JSON.stringify(
				error
			)}`
		);
